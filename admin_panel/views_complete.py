...
# ==================== RENTAL STATUS HISTORY MANAGEMENT ====================



@staff_member_required

def rental_status_history_list(request):

    """List rental status history"""

    history = RentalStatusHistory.objects.all().select_related('rental', 'changed_by').order_by('-created_at')
    ...
    if date_str:
        ...
        try:
            ...
            history = history.filter(created_at__date=date_obj)
    ...
    return render(request, 'admin_panel/rental_status_history_list.html', context)



@staff_member_required

def rental_feedback_respond(request, pk):
